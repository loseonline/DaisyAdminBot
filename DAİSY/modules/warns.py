...
import telegram
from DAİSY import TIGERS, WOLVES, dispatcher
...
from DAİSY.modules.helper_funcs.extraction import (
    extract_text,
    extract_user,
    extract_user_and_text,
)
from DAİSY.modules.helper_funcs.filters import CustomFilters
from DAİSY.modules.helper_funcs.misc import split_message
from DAİSY.modules.helper_funcs.string_handling import split_quotes
from DAİSY.modules.log_channel import loggable
from DAİSY.modules.sql import warns_sql as sql
...
from DAİSY.modules.sql.approve_sql import is_approved
...
def warn(user: User,
    ...
    if num_warns >= limit:
        ...
        keyboard = None
        log_reason = (f"{html.escape(chat.title)}:\n"
                      f"#WARN_BAN\n"
                      f"Admin: {warner_tag}\n"
                      f"User: {mention_html(user.id, user.first_name)}\n"
                      f"Reason: {reason}\n"
                      f"Counts: {num_warns}/{limit}")
...
def reset_warns(update: Update, context: CallbackContext) -> str:
    ...
    if user_id:
        ...
        return (f"{html.escape(chat.title)}:\n"
                f"#RESETWARNS\n"
                f"Admin: {mention_html(user.id, user.first_name)}\n"
                f"User: {mention_html(warned.id, warned.first_name)}")
    else:
        ...
        if args[0].isdigit():
            if int(args[0]) < 3:
                ...
                return (
                    ...
                    f"#SET_WARN_LIMIT\n"
                    f"Admin: {mention_html(user.id, user.first_name)}\n"
                    f"Set the warn limit to {args[0]}")
        else:
